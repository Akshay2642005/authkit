...
impl TokenStrategy for DatabaseTokenStrategy {
	...
	async fn verify_token(
		...
		let db_token = db
			...
			.await?
			.ok_or_else(|| AuthError::InvalidToken(format!("Token not found or invalid")))?;

		// Check if token has already been used
		if db_token.used_at.is_some() {
			return Err(AuthError::TokenAlreadyUsed(format!(
		...
		if db_token.expires_at < now {
			return Err(AuthError::TokenExpired(format!("Token has expired")));
		}
